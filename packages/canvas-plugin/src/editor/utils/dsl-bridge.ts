...
//
// The Intellect agent DSL has exactly one canonical wire shape, used
...
import { DataflowEmptyDsl } from '@agentui/canvas-plugin/editor/empty-dsl';
...
const normalizeGraphNodes = (nodes: IntellectNodeType[]): IntellectNodeType[] =>
  nodes.map((node) => {
    if (
...
export const importDsl = (
  ...
  // seed.
  let graph: { nodes: IntellectNodeType[]; edges: Edge[] };
  ...
  if (Array.isArray(rawParsed?.graph?.nodes)) {
    ...
    graph = {
      nodes: normalizeGraphNodes(rawParsed.graph.nodes as IntellectNodeType[]),
      edges,
    };
    components =
...
export const dslToGraph = (
  dsl: DSL,
): { nodes: IntellectNodeType[]; edges: Edge[] } => {
  ...
  if (Array.isArray(graphNodes) && graphNodes.length > 0) {
    ...
    return {
      nodes: normalizeGraphNodes(graphNodes as IntellectNodeType[]),
