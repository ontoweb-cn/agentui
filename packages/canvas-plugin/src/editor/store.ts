...
export type RFState = {
  ...
  onConnect: OnConnect;
  setNodes: (nodes: IntellectNodeType[]) => void;
  ...
  addNode: (nodes: IntellectNodeType) => void;
  getNode: (id?: string | null) => IntellectNodeType | undefined;
  updateNode: (node: IntellectNodeType) => void;
  ...
  findNodeByName: (operatorName: Operator) => IntellectNodeType | undefined;
  ...
  findUpstreamNodeById: (id?: string | null) => IntellectNodeType | undefined;
...
const useGraphStore = create<RFState>()(
  devtools(
    immer((set, get) => ({
      ...
      onNodesChange: (changes) => {
        set({
          nodes: applyNodeChanges(
            ...
            cloneDeep(get().nodes) as IntellectNodeType[], //   Cannot assign to read only property 'width' of object '#<Object>'
          ),
        });
      },
      ...
      setNodes: (nodes: IntellectNodeType[]) => {
        set({ nodes });
      },
      setEdges: (edges: Edge[]) => {
        set({ edges });
      ...
      addNode: (node: IntellectNodeType) => {
      ...
      duplicateIterationNode: (id: string, name: string) => {
        ...
        const iterationNode: IntellectNodeType = {
          ...(node || {}),
          data: {
      ...
      generateAgentToolName: (id: string, name: string) => {
        const node = get().nodes.find((x) => x.id === id) as IntellectNodeType;
      ...
      getAgentToolById: (
        id: string,
        nodeOrNodeId?: IntellectNodeType | string,
      ) => {
