/**
 * ScenarioKGView 知识图谱（共享视图组件）。
 *
 * 供导航台（导演台）Tab 与独立 KGViewPage 复用；
...
export const ScenarioKGView: React.FC<{ scenarioId: string }> = ({
  scenarioId,
}) => {
  ...
  const handleSearchChain = async () => {
    ...
    setChainError(null);
    try {
      const raw = await api.getNarrativeChain(
        scenarioId,
        chainInput.trim(),
        maxHops,
      );
      ...
      <Tabs defaultValue="entities">
        <TabsList>
          <TabsTrigger value="entities">
            {t('cognitiveWargame.kg.entities')} /{' '}
            {t('cognitiveWargame.kg.relations')}
          </TabsTrigger>
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead>
                          {t('cognitiveWargame.kg.betweenness')}
                        </TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
