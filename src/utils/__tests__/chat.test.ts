...
describe('preprocessLaTeX', () => {
  ...
  it('converts inline math ending in a letter variable', () => {
    // Regression: the previous `(?<![a-zA-Z])` lookbehind before `\)` rejected
    // any equation whose last token was a single-letter variable, because the
    // variable letter was mistaken for a LaTeX command name. `\(x < y\)` must
    // convert to `$x < y$`.
    expect(preprocessLaTeX('\\(x < y\\)')).toBe('$x < y$');
  });
});
