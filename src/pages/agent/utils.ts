  ...
  >((pre, cur) => {
    if (cur.fileFormat) {
      ...
      > = {
        ...
        preprocess: (cur as any).preprocess,
...
export const duplicateNodeForm = (nodeData?: IntellectNodeType['data']) => {
...
export function getAgentNodeTools(agentNode?: IntellectNodeType) {
  const tools: IAgentForm['tools'] = get(agentNode, 'data.form.tools', []);
  return tools;
}

export function getAgentNodeMCP(agentNode?: IntellectNodeType) {
