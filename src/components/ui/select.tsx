...
export type IntellectSelectOptionType = {
...
export type IntellectSelectGroupOptionType = {
  label: React.ReactNode;
  options: IntellectSelectOptionType[];
};

export type IntellectSelectProps = Partial<ControllerRenderProps> & {
  FormControlComponent?: typeof FormControl;
  options?: (IntellectSelectOptionType | IntellectSelectGroupOptionType)[];
  allowClear?: boolean;
 ...
 * @return {*}
 */
export const IntellectSelect = forwardRef<
  React.ElementRef<typeof SelectPrimitive.Trigger>,
  IntellectSelectProps
>(function (
  {
    value: initialValue,
    onChange,
  ...
  const label = React.useMemo(() => {
    ...
    if (options.some((x) => !('value' in x))) {
      nextOptions = (options as IntellectSelectGroupOptionType[]).reduce<
        IntellectSelectOptionType[]
      >((pre, cur) => {
        return pre.concat(cur?.options ?? []);
      }, []);
    }

    const option = (nextOptions as IntellectSelectOptionType[]).find(
      (x) => x.value === value,
    );
      ...
      <SelectContent {...contentProps}>
        {options.map((o, idx) => {
          if ('value' in o) {
            return (
              <SelectItem
                value={o.value as IntellectSelectOptionType['value']}
                key={o.value}
                disabled={o.disabled}
                data-testid={
                  optionTestIdPrefix
...
IntellectSelect.displayName = 'IntellectSelect';
