...
例如：一条带有 1024 维嵌入的 1 KB 消息大约使用 9 KB。5 MB 的默认限制大约可容纳 500 条此类消息。`,
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        '进行检索测试，检查 Intellect 是否能够为大语言模型（LLM）恢复预期的内容。',
      Parse: '解析',
      dataset: '知识库',
      testing: '检索测试',
      configuration: '配置',
      ...
      parsingStatus: '解析状态',
      parsingStatusTip:
        '文本解析的时间取决于诸多因素。如果开启了知识图谱、RAPTOR、自动问题提取、自动关键词提取等功能，时间会更长。如果解析进度条长时间不更新，也可以参考这两条 FAQ：https://intellect.ontoweb.cn/docs/dev/faq#why-does-my-document-parsing-stall-at-under-one-percent。',
      processBeginAt: '开始于',
      ...
      layoutRecognizeTip:
        '使用视觉模型进行 PDF 布局分析，以更好地识别文档结构，找到标题、文本块、图像和表格的位置。 如果选择 Naive 选项，则只能获取 PDF 的纯文本。请注意该功能只适用于 PDF 文档，对其他文档不生效。欲了解更多信息，请参阅 https://intellect.ontoweb.cn/docs/dev/select_pdf_parser。',
      taskPageSize: '任务页面大小',
      ...
      uploadDescription:
        '支持单次或批量上传。本地部署的单次上传文件总大小上限为 1GB，单次批量上传文件数不超过 32，单个账户不限文件数量。对于 cloud.intellect.ontoweb.cn：每次上传的总文件大小限制为 10MB，每个文件不得超过 10MB，每个账户最多可上传 128 个文件。严禁上传违禁文件。',
      chunk: '解析块',
      bulk: '批量',
      cancel: '取消',
      close: '关闭',
      ...
      rerankTip: `非必选项：若不选择 rerank 模型，系统将默认采用关键词相似度与向量余弦相似度相结合的混合查询方式；如果设置了 rerank 模型，则混合查询中的向量相似度部分将被 rerank 打分替代。请注意：采用 rerank 模型会非常耗时。如需选用 rerank 模型，建议使用 SaaS 的 rerank 模型服务；如果你倾向使用本地部署的 rerank 模型，请务必确保你使用 docker-compose-gpu.yml 启动 Intellect。`,
      topK: 'Top-K',
      ...
      html4excel: '表格转HTML',
      html4excelTip: `与 General 切片方法配合使用。未开启状态下，表格文件（XLSX、XLS（Excel 97-2003））会按行解析为键值对。开启后，表格文件会被解析为 HTML 表格。若原始表格超过 12 行，系统会自动按每 12 行拆分为多个 HTML 表格。欲了解更多详情，请参阅 https://intellect.ontoweb.cn/docs/dev/enable_excel2html。`,
      autoKeywords: '自动关键词提取',
      autoKeywordsTip: `自动为每个文本块中提取 N 个关键词，用以提升查询精度。请注意：该功能采用在“配置”中指定的索引模型提取关键词，因此也会产生更多 Token 消耗。另外，你也可以手动更新生成的关键词。详情请见 https://intellect.ontoweb.cn/docs/dev/autokeyword_autoquestion。`,
      autoQuestions: '自动问题提取',
      autoQuestionsTip: `利用在“配置”中指定的索引模型 对知识库的每个文本块提取 N 个问题以提高其排名得分。请注意，开启后将消耗额外的 token。您可以在块列表中查看、编辑结果。如果自动问题提取发生错误，不会妨碍整个分块过程，只会将空结果添加到原始文本块。详情请见 https://intellect.ontoweb.cn/docs/dev/autokeyword_autoquestion。`,
...
`,
    ...
    <p>
      ...
      您只需与<i>'Intellect'</i>交谈即可列出所有符合资格的候选人。
      </p>
        `,
...
`,
      ...
      useRaptorTip:
        'RAPTOR 常应用于复杂的多跳问答任务。如需打开，请跳转至知识库的文件页面，点击生成 > RAPTOR 开启。详见: https://intellect.ontoweb.cn/docs/dev/enable_raptor。',
      prompt: '提示词',
...
以上就是你需要总结的内容。`,
      ...
      entityTypes: '实体类型',
      pageRank: '页面排名',
      pageRankTip: `知识库检索时，你可以为特定知识库设置较高的 PageRank 分数，该知识库中匹配文本块的混合相似度得分会自动叠加 PageRank 分数，从而提升排序权重。详见 https://intellect.ontoweb.cn/docs/dev/set_page_rank。`,
      tagName: '标签',
      frequency: '频次',
      searchTags: '搜索标签',
      tagCloud: '云',
      ...
      <p> 详见：https://intellect.ontoweb.cn/docs/dev/use_tag_sets </p>
      `,
      tags: '标签',
      addTag: '增加标签',
      useGraphRag: '提取知识图谱',
      useGraphRagTip:
        '基于知识库内所有切好的文本块构建知识图谱，用以提升多跳和复杂问题回答的正确率。请注意：构建知识图谱将消耗大量 token 和时间。详见 https://intellect.ontoweb.cn/docs/dev/construct_knowledge_graph。',
      graphRagMethod: '方法',
...
**重要规则:**
  ...
  - **始终考虑**整个对话历史。`,
      ...
      “系统提示词”中的所有变量都必须用大括号{}括起来。详见 https://intellect.ontoweb.cn/docs/dev/set_chat_variables。`,
      add: '新增',
      key: '关键字',
      optional: '可选的',
      operation: '操作',
...
示例：general/v2/`,
      ...
      ttsModel: 'TTS',
      ttsModelTip:
        '默认的tts模型会被用于在对话过程中请求语音生成时使用。如未显示可选模型，请根据 https://intellect.ontoweb.cn/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      embeddingModel: 'Embedding',
      embeddingModelTip:
        '所有新创建的知识库使用的默认嵌入模型。如未显示可选模型，请检查你是否在使用 Intellect slim 版(不含嵌入模型)；或根据 https://intellect.ontoweb.cn/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      img2txtModel: 'VLM',
      img2txtModelTip:
        '所有新创建的知识库都将使用默认的 img2txt 模型。 它可以描述图片或视频。如未显示可选模型，请根据 https://intellect.ontoweb.cn/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      sequence2txtModel: 'ASR',
      sequence2txtModelTip:
        '所有新创建的知识库都将使用默认的 ASR 模型。 使用此模型将语音翻译为相应的文本。如未显示可选模型，请根据 https://intellect.ontoweb.cn/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      rerankModel: 'Rerank',
      rerankModelTip: `默认的 reranking 模型。如未显示可选模型，请根据 https://intellect.ontoweb.cn/docs/dev/supported_models 确认你的模型供应商是否提供该模型。`,
      workspace: '工作空间',
      upgrade: '升级',
      addLlmTitle: '添加 LLM',
...
[在此处插入文本]`,
    ...
    // spec-010 B-6: Wizard 首次安装向导
    wizard: {
      title: '安装向导',
      subtitle: '连接你的第一个 harness 后端',
      step: {
        welcome: '欢迎',
        selectType: '选择类型',
        connection: '连接信息',
      ...
      welcome: {
        title: '欢迎使用安装向导',
        description:
          '本向导将引导你连接第一个 harness 后端。你可以配置 Intellect Community、Intellect Enterprise 或其他 OpenAI 兼容后端。',
        steps: '流程:选择后端类型 → 填写连接信息 → 探测 → 确认 → 完成。',
        note: 'Token 仅通过 HTTPS 传输,响应不含明文 Token;系统会提供一段 .env 片段用于持久化保存。',
        start: '开始配置',
      },
      selectType: {
        title: '选择后端类型',
        description: '选择你要连接的 harness 后端平台。',
      ...
      connection: {
        title: '连接信息',
        description: '填写后端端点和凭据。Token 通过 HTTPS 传输,不会以明文存储。',
        name: '后端名称',
        endpoint: '端点',
        adminTokenEnvVar: 'Admin Token 环境变量',
        adminTokenEnvVarHint: '运行时从该环境变量读取 Token(无 vault 时的回退方案)。',
        token: 'Bearer Token',
        email: '邮箱',
        password: '密码',
        intellectTenantId: 'Intellect 租户 ID',
        intellectTenantIdError: '格式错误:必须为 32 位 hex(0-9, a-f)',
        intellectTenantIdHint: '从 intellect-team 的 INTELLECT_TENANT_ID 环境变量复制(Rust 版本要求 32 位 hex)',
        probe: '探测连接',
      },
      probe: {
        title: '连接探测',
        description: '正在校验后端可达性并探测能力。',
        probing: '探测中...',
        healthy: '后端可达',
        capabilities: '已发现能力',
        failed: '探测失败',
      ...
      confirm: {
        title: '确认配置',
        description: '请检查配置。确认后将创建后端配置。',
        name: '名称',
        type: '类型',
        endpoint: '端点',
        envVar: '环境变量',
        tenantId: '租户 ID',
        envSnippet: '.env 片段(复制到你的 .env 文件)',
        error: '错误',
        submit: '创建后端',
      },
      done: {
        title: '配置完成',
        description: '后端已成功配置,现在可以开始使用平台。',
        goHome: '返回首页',
      },
      toast: {
        success: '后端创建成功',
