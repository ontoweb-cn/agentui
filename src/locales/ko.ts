  ...
  translation: {
    ...
    knowledgeDetails: {
      ...
      retrievalTestingDescription:
        'Intellect가 LLM에 전달하고자 하는 내용을 정확히 가져올 수 있는지 확인하는 검색 테스트를 실행하세요.',
      Parse: '파싱',
      dataset: '데이터셋',
      testing: '검색 테스트',
      files: '파일',
      ...
      parsingStatus: '파싱 상태',
      parsingStatusTip:
        '문서 파싱 시간은 여러 요소에 따라 달라집니다. 지식 그래프, RAPTOR, 자동 질문 추출, 자동 키워드 추출 등의 기능을 활성화하면 처리 시간이 크게 늘어납니다. 진행 표시줄이 멈추면 다음 FAQ를 참고하세요: https://intellect.ontoweb.cn/docs/dev/faq#why-does-my-document-parsing-stall-at-under-one-percent.',
      ...
      testingDescription:
        'Intellect가 LLM에 전달하고자 하는 내용을 정확히 가져올 수 있는지 확인하는 검색 테스트를 실행하세요. 키워드 유사도 가중치나 유사도 임계값 등 기본 설정을 조정하여 최적의 결과를 얻었다면, 해당 변경 사항은 자동으로 저장되지 않으므로 채팅 어시스턴트 설정 또는 검색 에이전트 컴포넌트 설정에 적용해야 합니다.',
      similarityThreshold: '유사도 임계값',
      similarityThresholdTip:
        'Intellect는 검색 시 가중 키워드 유사도와 가중 벡터 코사인 유사도, 또는 가중 키워드 유사도와 가중 rerank 점수의 조합을 사용합니다. 이 파라미터는 사용자 쿼리와 chunk 간 유사도 임계값을 설정합니다.\n이 임계값보다 낮은 유사도 점수를 가진 chunk는 결과에서 제외됩니다. 기본 임계값은 0.2이며, 하이브리드 유사도 점수가 20 이상인 chunk만 검색됩니다.',
      vectorSimilarityWeight: '벡터 유사도 가중치',
      ...
      uploadDescription:
        '단일 또는 일괄 파일 업로드를 지원합니다. 로컬 배포 Intellect의 경우: 업로드당 총 파일 크기 제한은 1GB이며, 일괄 업로드 제한은 32개 파일입니다. 계정당 총 파일 수에는 제한이 없습니다. cloud.intellect.ontoweb.cn의 경우: 업로드당 총 파일 크기 제한은 10MB이며, 각 파일은 10MB를 초과할 수 없고 계정당 최대 128개 파일입니다.',
      chunk: 'Chunk',
      bulk: '일괄',
      cancel: '취소',
      close: '닫기',
      ...
      rerankTip: `선택 사항입니다. 비워두면 Intellect는 가중 키워드 유사도와 가중 벡터 코사인 유사도의 조합을 사용합니다. rerank 모델을 선택하면 가중 rerank 점수가 가중 벡터 코사인 유사도를 대체합니다. rerank 모델을 사용하면 시스템 응답 시간이 크게 증가합니다. rerank 모델을 사용하려면 SaaS reranker를 사용하거나, 로컬 배포 rerank 모델을 선호하는 경우 docker-compose-gpu.yml로 Intellect를 시작해야 합니다.`,
      topK: 'Top-K',
      ...
      html4excelTip: `일반 chunking 방법과 함께 사용합니다. 비활성화 시 데이터셋의 스프레드시트(XLSX 또는 XLS(Excel 97-2003))는 키-값 쌍으로 파싱됩니다. 활성화 시 HTML 표로 파싱되며, 원래 표가 12행을 초과하면 12행마다 분리됩니다. 자세한 내용은 https://intellect.ontoweb.cn/docs/dev/enable_excel2html 을 참조하세요.`,
      autoKeywords: '자동 키워드',
      autoKeywordsTip: `각 chunk에서 N개의 키워드를 자동으로 추출하여 해당 키워드가 포함된 쿼리에서의 순위를 높입니다. '설정'에서 지정된 인덱싱 모델이 추가 토큰을 소비합니다. chunk 목록에서 추가된 키워드를 확인하거나 업데이트할 수 있습니다. 자세한 내용은 https://intellect.ontoweb.cn/docs/dev/autokeyword_autoquestion 을 참조하세요.`,
      autoQuestions: '자동 질문',
      autoQuestionsTip: `각 chunk에서 N개의 질문을 자동으로 추출하여 해당 질문이 포함된 쿼리에서의 순위를 높입니다. chunk 목록에서 추가된 질문을 확인하거나 업데이트할 수 있습니다. 오류가 발생해도 chunking 과정에 영향을 주지 않으나, 원래 chunk에 빈 결과가 추가될 수 있습니다. '설정'에서 지정된 인덱싱 모델이 추가 토큰을 소비합니다. 자세한 내용은 https://intellect.ontoweb.cn/docs/dev/autokeyword_autoquestion 을 참조하세요.`,
...
`,
    ...
    knowledgeConfiguration: {
      ...
      tableColumnModeAutoDescription:
        '모든 컬럼이 chunk 텍스트에 포함되고 메타데이터로 저장됩니다 (Intellect 기본값).',
...
`,
      useRaptor: 'RAPTOR',
      useRaptorTip:
        'RAPTOR는 멀티홉 질의응답 작업에 사용할 수 있습니다. 파일 탭에서 [생성 > RAPTOR]를 클릭하여 활성화하세요. 자세한 내용은 https://intellect.ontoweb.cn/docs/dev/enable_raptor 을 참조하세요.',
      prompt: '프롬프트',
      promptTip:
...
위 내용을 요약해 주세요.`,
      ...
      vietnamese: 'Tiếng Việt',
      pageRank: 'Page rank',
      pageRankTip: `검색 시 특정 데이터셋에 더 높은 PageRank 점수를 부여할 수 있습니다. 해당 점수는 이 데이터셋에서 검색된 chunk의 하이브리드 유사도 점수에 추가되어 순위를 높입니다. 자세한 내용은 https://intellect.ontoweb.cn/docs/dev/set_page_rank 을 참조하세요.`,
      tagName: '태그',
      frequency: '빈도',
      searchTags: '태그 검색',
      tagCloud: '클라우드',
      ...
      tagSet: '태그 세트',
      tagSetTip: `
     <p> 데이터셋의 chunk에 자동 태그를 지정할 태그 데이터셋을 하나 이상 선택하세요. 자세한 내용은 https://intellect.ontoweb.cn/docs/dev/use_tag_sets 을 참조하세요.</p>
...
<ul>
  ...
  <li>자동 키워드 기능은 LLM에 의존하며 상당한 수의 토큰을 소비합니다.</li>
      ...
      useGraphRag: '지식 그래프',
      useGraphRagTip:
        '현재 데이터셋의 파일 청크에 대해 지식 그래프를 구성하여, 중첩된 논리가 포함된 멀티홉 질의응답(multi-hop QA) 성능을 향상시킵니다. 자세한 내용은 https://intellect.ontoweb.cn/docs/dev/construct_knowledge_graph 을 참조하세요.',
      graphRagMethod: '방법',
      graphRagMethodTip: `
...
**필수 규칙:**
  ...
  - **항상** 전체 대화 기록을 고려하세요.`,
      ...
      **필수 규칙:**
      ...
      variable: '변수',
      variableTip: `Intellect의 채팅 어시스턴트 관리 API와 함께 사용하면 변수를 통해 더 유연한 시스템 프롬프트 전략을 개발할 수 있습니다. 정의된 변수는 '시스템 프롬프트'에서 LLM의 프롬프트 일부로 사용됩니다. {knowledge}는 지정된 데이터셋에서 검색된 chunk를 나타내는 예약 특수 변수이며, 모든 변수는 '시스템 프롬프트'에서 중괄호 {}로 묶어야 합니다. 자세한 내용은 https://intellect.ontoweb.cn/docs/dev/set_chat_variables 를 참조하세요.`,
      add: '추가',
      key: '키',
      optional: '선택 사항',
      operation: '작업',
      ...
      betaError: '시스템 설정 페이지에서 Intellect API 키를 먼저 획득해 주세요.',
      searching: '검색 중...',
      parsing: '파싱 중',
      uploading: '업로드 중',
      uploadFailed: '업로드 실패',
    ...
    setting: {
      ...
      embeddingModelTip:
        '새로 생성된 각 데이터셋의 기본 임베딩 모델입니다. 드롭다운에서 임베딩 모델을 찾을 수 없으면 Intellect 슬림 에디션(임베딩 모델 미포함)을 사용 중인지 확인하거나 https://intellect.ontoweb.cn/docs/dev/supported_models 에서 모델 제공업체가 이 모델을 지원하는지 확인하세요.',
      img2txtModel: 'VLM',
      img2txtModelTip:
        '새로 생성된 각 데이터셋의 기본 VLM입니다. 이미지나 동영상을 설명합니다. 드롭다운에서 모델을 찾을 수 없으면 https://intellect.ontoweb.cn/docs/dev/supported_models 에서 모델 제공업체가 이 모델을 지원하는지 확인하세요.',
      sequence2txtModel: 'ASR',
      sequence2txtModelTip:
        '새로 생성된 각 데이터셋의 기본 ASR 모델입니다. 이 모델을 사용하여 음성을 텍스트로 변환하세요.',
      rerankModel: 'Rerank',
      rerankModelTip: `Chunk rerank를 위한 기본 rerank 모델입니다. 드롭다운에서 모델을 찾을 수 없으면 https://intellect.ontoweb.cn/docs/dev/supported_models 에서 모델 제공업체가 이 모델을 지원하는지 확인하세요.`,
      ttsModel: 'TTS',
      ttsModelTip:
        '기본 텍스트 음성 변환 모델입니다. 드롭다운에서 모델을 찾을 수 없으면 https://intellect.ontoweb.cn/docs/dev/supported_models 에서 모델 제공업체가 이 모델을 지원하는지 확인하세요.',
      workspace: '워크스페이스',
      upgrade: '업그레이드',
      addLlmTitle: 'LLM 추가',
