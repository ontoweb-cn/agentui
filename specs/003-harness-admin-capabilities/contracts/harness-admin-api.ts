 ...
 * Runtime: bff/src/routes/harness-admin.ts + bff/src/routes/capabilities.ts (P2 实现)
 *
...
import type { BackendType, HarnessCapabilities, HarnessBackendConfig } from './harness';
...
/**
 * Harness Admin API — 后端配置 CRUD。
 * 挂载点:/api/bff/admin/harness-backends(Vite rewrite 后 BFF 收到 /admin/harness-backends)
 * 鉴权:authMiddleware(需 Authorization header,非租户隔离)
 */
export interface HarnessAdminAPI {
  /** 列出所有后端配置 + ready 状态 */
  list: {
    method: 'GET';
    path: '/admin/harness-backends';
    response: { code: 0; data: HarnessBackendWithStatus[] };
  };
  /** 新增后端(校验 id 唯一/格式) */
  create: {
    method: 'POST';
    path: '/admin/harness-backends';
    body: HarnessBackendForm;
    response: { code: 0; data: HarnessBackendWithStatus } | { code: 409; message: string };
  };
  /** 编辑后端(id 只读,用路径参数) */
  update: {
    method: 'PUT';
    path: '/admin/harness-backends/:id';
    body: Omit<HarnessBackendForm, 'id'>;
    response: { code: 0; data: HarnessBackendWithStatus } | { code: 404; message: string };
  };
  /** 删除后端(校验未绑定) */
  remove: {
    method: 'DELETE';
    path: '/admin/harness-backends/:id';
    response: { code: 0; message: string } | { code: 409; message: string };
  };
}

/**
 * Capabilities API — 能力探测。
 * 挂载点:/api/bff/capabilities(Vite rewrite 后 BFF 收到 /capabilities)
 * 鉴权:authMiddleware + tenantContextMiddleware(按 X-Tenant-Id 返回)
 */
export interface CapabilitiesAPI {
  ...
  get: {
    ...
    response:
      | { code: 0; data: CapabilitiesResponse }
      | { code: 404; message: string } // tenant 不存在
      | { code: 503; message: string }; // Registry 未就绪
  };
}
