...
import type { BffTenant } from './tenant-context';
...
export interface TenantStore {
   ...
   * @param backendId 后端 ID(必须已存在于 HarnessStore)
  ...
  /**
   * 获取 Tenant 的主后端 ID。
   * @returns 主后端 ID,未绑定则返回 undefined
   */
   ...
   *
   * @param tenantId Tenant ID
   * @param backendId 画布后端 ID(必须是 intellect-rag 类型)
  ...
  /**
   * 获取 Tenant 的画布后端 ID。
   * @returns 画布后端 ID,未绑定则返回 undefined
   */
