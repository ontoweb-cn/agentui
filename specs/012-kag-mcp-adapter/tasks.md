...
- [ ] **任务**: 安装 `@modelcontextprotocol/sdk` TypeScript SDK
- [ ] **文件**: `bff/package.json`
- [ ] **实施细节**:
  - `cd bff && npm install @modelcontextprotocol/sdk`
  - 锁定主版本(`^1.0.0`),避免 minor/patch 升级破坏 API
- [ ] **验收**: `npm ls @modelcontextprotocol/sdk` 显示已安装,tsc 编译无错误

### P1-2:MCPBaseAdapter 抽象基类

- [ ] **任务**: 实现 MCP 协议 Adapter 抽象基类
- [ ] **文件**: `bff/src/services/adapters/shared/mcp-base-adapter.ts`
- [ ] **实施细节**(对齐 spec-012 §2.2):
  ...
  - SSRF 防护:复用 `validateEndpoint()` 校验
  - 超时:30s(AbortSignal.timeout)
- [ ] **验收**: 基类可被继承,抽象方法清晰

### P1-3:IMCPAdapter 接口 + MCPTool 类型

- [ ] **任务**: 定义 MCP 扩展接口和工具描述类型
- [ ] **文件**: `bff/src/types/adapter.ts`
- [ ] **实施细节**(对齐 spec-012 §3.1):
...
- [ ] **验收**: 接口编译通过,isMCPAdapter 守卫可用

### P1-4:KagAdapter 实现

- [ ] **任务**: 实现 KAG 后端 Adapter
- [ ] **文件**: `bff/src/services/adapters/kag/kag-adapter.ts`
- [ ] **实施细节**(对齐 spec-012 §4.1):
...
- [ ] **验收**: KagAdapter 实例可被 isMCPAdapter() 守卫通过
...
- [ ] **任务**: 全套 BFF 测试 0 回归
- [ ] **命令**: `cd bff && npm test`
- [ ] **验收**: 所有现有测试通过(620 + 新增 KAG 测试),0 回归
